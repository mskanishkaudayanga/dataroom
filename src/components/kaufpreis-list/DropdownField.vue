...
export default {
    ...
    methods: {
        ...
        handleMultiSelections(object) {
            if (this.restrictedMultiSelectCount === 0) {
                if (object.selected) {
                    let filteredIndex = this.selectedProjectsInMultiSelection.findIndex(item => item.value === object.value);
                ...
                if (object.selected) {
                    ...
                    if (this.selectedProjectsInMultiSelection.length < this.restrictedMultiSelectCount) {
                        object.selected = true;
                        this.selectedProjectsInMultiSelection.push(object);
                    }  else {
                        return;
    ...
    computed: {
        filterItems() {
            return this.items.filter(item => item.title.toLowerCase().includes(this.inputValue.toLowerCase()));
        },
    },
...
<template>
    <template v-if="isModal">
        ...
        <span @mouseover="showTooltip = true" @mouseleave="showTooltip = false">
            <input type="text" autocomplete="off" v-model="inputDataModal" :placeholder="placeHolder" :class="isModal ? 'kpl-dropdown-input-in-modal' : 'kpl-dropdown-input'"
            @click.stop="onDropDownInputClicked" @input="onDropDownSearch" :disabled="disabled" />
            <i class="fa-solid fa-caret-down dropdown-arrow"></i>

            <div v-if="disabled && showTooltip" class="tooltip">
                {{ disableMessage }}
            </div>
        </span>
    </template>
    ...
    <div :class="dropDownClass">
        <ul @click.stop>
            <li v-for="(object, index) in filterItems" v-on:click="onDropdownChange(object)" :class="object.selected ? 'selected' : ''">
                <span>
                    {{ object.title }}
                </span>
...
.dropdown-arrow {
    position: absolute;
    color: var(--dark-gray)e !important;
    right: 10px;
    top: 50%;
