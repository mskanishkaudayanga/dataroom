...
export default {
    ...
    computed: {
        filterItems() {
            return this.items.filter(item => item.selection_label.toLowerCase().includes(this.inputValue.toLowerCase()));
        },
    },
...
<template>
    <span class=".dropdown-container">
        <input type="text" autocomplete="off" v-model="inputModel" :placeholder="placeHolder" :class="isModal ? 'kpl-dropdown-input-in-modal' : 'kpl-dropdown-input'"
        @click.stop="onDropDownInputClicked" @input="onDropDownSearch" :style="{ paddingRight: '20px' }"/>
