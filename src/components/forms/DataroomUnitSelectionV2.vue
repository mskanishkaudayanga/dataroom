...
export default {
  ...
  methods: {
    ...
    this.isEmpty(this.selectedProjectData) ||
    this.countObjectKeys(this.selectedProjectData) === 0
  ) {
      ...
      axios
        ...
        .then((response) => {
          if (response.status === 200) {
            ...
            const selectedProjectArray = Object.entries(
            ...
            for (let index in selectedProjectArray) {
              ...
              if (
                this.isStammobjectInDataset(
                  this.selectedProjectData[projectName].nr
                )
              ) {
    ...
    updateMatchingProjects(data) {
      ...
      for (let projectName in this.matchingRealEstates[this.searchCriteriaId]) {
        ...
        let existingMatchingUnits =
          this.matchingRealEstates[this.searchCriteriaId][projectName].units;
        for (let unitIndex in existingMatchingUnits) {
          allMatchingUnits.push(
            existingMatchingUnits[unitIndex]["dataset"]["nr"]
          );
        }
        this.selectedProjectData[projectName]["nr"] = this.selectedProjectData[
          projectName
        ]["nr"].filter(
    ...
    getRealEstateData(realEstateNr, key) {
      for (let projectName in this.projectAndUnitDatalist) {
        for (let realEstates in this.projectAndUnitDatalist[projectName]) {
          if (
            this.projectAndUnitDatalist[projectName][realEstates].dataset.nr ===
            realEstateNr
          ) {
            if (
              key === "floor" &&
              typeof this.projectAndUnitDatalist[projectName][realEstates]
                .dataset["floor_string"] !== "undefined"
            ) {
              return this.projectAndUnitDatalist[projectName][realEstates]
                .dataset["floor_string"];
            }

            if (key === "price") {
              if (
                !Number.isInteger(
                  this.projectAndUnitDatalist[projectName][realEstates].dataset[
                    key
                  ]
                )
              ) {
    ...
    checkIfIsSpainProject(projectName) {
      ...
      for (let index in this.projectAndUnitDatalist[projectName]) {
        if (
          this.projectAndUnitDatalist[projectName][index].dataset.country ===
          "Spanien"
        ) {
          returnVal = true;
    ...
    addStammobject(projectName) {
      const getRealEstatesWithCheckedFloorplans = this.matchingRealEstates[
        this.searchCriteriaId
      ][projectName].units.filter(
        (item) =>
          item.dataset && item.dataset.real_estate_floor_checkbox === true
      );
      const nrs = getRealEstatesWithCheckedFloorplans.map(
        (obj) => obj.dataset.nr
      );
      const stammobject = this.matchingRealEstates[this.searchCriteriaId][
        projectName
      ].units.filter(
        (item) => item.dataset && this.isStringStammobject(item.dataset.nr)
      );
    ...
    removeStammobject(projectName) {
      ...
      if (typeof stammobject[0] !== "undefined") {
        const filteredArray = this.selectedProjectData[projectName].nr.filter(
          (item) => item !== stammobject[0].dataset.nr
        );
...
<template>
  <div class="loading-screen" v-if="isLoading && countObjectKeys(selectedProjectData) > 0 && searchCriteriaId">
  <AnimateCircleIcon></AnimateCircleIcon>
</div>


<div v-if="isEmpty(selectedProjectData) || countObjectKeys(selectedProjectData) === 0" class="empty-state">
  <p>Keine Projekte ausgewählt. Bitte wählen Sie zuerst Projekte aus.</p>
</div>


  <div
    class="selection"
    v-for="(itemData, projectName) in selectedProjectData"
    v-if="
      countObjectKeys(selectedProjectData) > 0 &&
      typeof matchingRealEstates[searchCriteriaId] !== 'undefined'
    "
  >
    <div v-if="checkIfIsSpainProject(projectName)">
      Für spanische Projekte können keine Einheiten ausgewählt werden.
    </div>
    <div
      class="field-wrapper"
      v-else-if="
        itemData.selected &&
        typeof matchingRealEstates[searchCriteriaId][projectName] !==
          'undefined'
      "
    >
      <seperator
        >{{ projectName }}
        {{ selectedProjectData[projectName].length }} Einheiten</seperator
      >

      <div class="units">
        <label
          :for="
            projectName.toLocaleLowerCase().replaceAll(' ', '_') + '-selected'
          "
        >
          <input
        ...
        <label
          :for="unitData.dataset.nr.toLocaleLowerCase() + '-selected'"
          :class="{
            hide: unitData.dataset.stammobject,
            inactive: selectedStammobjects.includes(projectName),
          }"
          v-for="unitData in matchingRealEstates[searchCriteriaId][projectName]
            .units"
        >
          <input
            class="checkbox field"
            style="margin-right: 10px"
            type="checkbox"
            :id="unitData.dataset.nr.toLocaleLowerCase() + '-selected'"
            :name="unitData.dataset.nr.toLocaleLowerCase() + '-selected'"
            :value="unitData.dataset.nr"
            v-model="selectedProjectData[projectName].nr"
          />
          <span class="unit-label">
            ...
            <span class="price"
              >{{
                this.numberWithComma(
                  getRealEstateData(unitData.dataset.nr, "price")
                )
              }}
              €</span
            >
            ...
            <span class="rooms"
              >{{
                getRealEstateData(unitData.dataset.nr, "rooms")
              }}
              Zimmer</span
            >
            <span
              ...
              v-if="
                !this.isEmpty(
                  this.getFloorMapping(
                    getRealEstateData(unitData.dataset.nr, 'floor')
                  )
                )
              "
              >|</span
