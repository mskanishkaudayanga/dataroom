<script>
  ...
  export default {
    name: "canbanModalLeadToUnqualified.vue",
  ...
  <div class="modal-wrapper" :class="{ active: isOpenModal }" v-if="isOpenModal" @click="closeDropdowns">
    ...
    <div class="modal-inner-wrapper">
      ...
      <div class="modal-header">
        <div class="title">Lead unqualifiziert markieren</div>
      ...
      <div class="modal-body">
        ...
        <div class="field-wrapper" :class="{'invalid': !denieReason.valid, 'valid': denieReason.valid }" v-if="typeOfDelete === 'denieLead'">
            <InputField class="dropdown-input" ref="inputSource" inputType="text" inputName="Source" :inputValue="denieReason.value" v-model="denieReason.value" @focus="setDropdownTop($event)" @click="dropdowns.reason = !dropdowns.reason" @click.stop="" data-cy="unqualified-reasons-input"></InputField>
            <label class="ph">Grund</label>
            <DropdownArrow class="dropdown-arrow"></DropdownArrow>
            <Dropdown ref="dropdownSource" :class="{active: dropdowns.reason, relative: dropdownRelativePosition}" type="country" :renderingObject="dropdownContents.reason" :filterFor="denieReason.value" @dropdown-item-clicked="dropdownClick('denieReason', $event)" @click.stop="" data-cy="unqualified-reasons"></Dropdown>
        </div>

        <div class="actions">
          <Button type="delete" @click="closeModal">Schließen</Button>
          <Button :type="!deleteProcess ? 'edit' : 'report'" @click="submitLeadDelete">{{ !deleteProcess ? (typeOfDelete === 'denieAppointment' ? 'Termin absagen' : 'Lead unqualifiziert') : 'Bestätigen' }} <AnimateCircleIcon v-if="this.isLoading"></AnimateCircleIcon></Button>
        </div>
