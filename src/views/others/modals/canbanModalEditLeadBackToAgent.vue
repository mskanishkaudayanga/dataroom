    ...
    methods: {
      sendLeadBackToAgent() {
        ...
        let dataToSend = {
          ...
          'viewName': this.routerName
        };

        axios.post(this.globalApiBaseUrl + '/canban/leadBackToAgent', dataToSend)
            .then(
                (response) => {
                  if (response.data.status === 200) {
                    this.triggerCanbanReload();
                    this.closeModal();
                    this.notificationBannerInit('success', 'Lead erfolgreich zurückgeschickt');
                  } else {
                    this.notificationBannerInit('warning', 'Lead konnte nicht zurückgeschickt werden');
                  }
                }
            )
            .catch(
      ...
      closeModal(){
        this.resetData();
        if ( this.routerName === 'canbanModalEditReactiveLead' ) {
          this.$router.push({name: 'canbanToolUnqualifiedLeads'})
        } else {
  ...
  <div class="modal-wrapper" :class="{ active: isOpenModal }" v-if="isOpenModal" @click="closeDropdowns">
    ...
    <div class="modal-inner-wrapper">
      ...
      <div class="modal-header">
        <div class="title">{{ routerName === 'canbanModalEditReactiveLead' ? 'Lead wieder aktivieren' : 'Lead zurück an Makler leiten' }}</div>
      ...
      <div class="modal-body">
        ...
        <div class="actions">
          ...
          <Button type="report" @click="sendLeadBackToAgent">{{ routerName === 'canbanModalEditReactiveLead' ? 'Lead reaktivieren' : 'Lead zurücksenden' }} <AnimateCircleIcon v-if="this.isLoading"></AnimateCircleIcon></Button>
        </div>
