  ...
  export default {
    ...
    methods: {
      submitLeadDelete() {
        ...
        axios.post(this.globalApiBaseUrl + '/canban/deleteLeadPermanently', dataToSend)
            .then(
                (response) => {
                  if (response.data.status === 200) {
                    this.triggerCanbanReload();
                    this.closeModal();
                    this.notificationBannerInit('success', 'Lead erfolgreich gelöscht');
                  } else {
                    this.notificationBannerInit('warning', 'Lead konnte nicht gelöscht werden');
                  }
                }
            )
            .catch(
  ...
  <div class="modal-wrapper" :class="{ active: isOpenModal }" v-if="isOpenModal" @click="closeDropdowns">
    ...
    <div class="modal-inner-wrapper">
      ...
      <div class="modal-header">
        <div class="title">Lead löschen</div>
      ...
      <div class="modal-body">
        ...
        <div class="actions">
          ...
          <Button :type="!deleteProcess ? 'edit' : 'report'" @click="submitLeadDelete">{{ !deleteProcess ? 'Lead löschen' : 'Bestätigen' }} <AnimateCircleIcon v-if="this.isLoading"></AnimateCircleIcon></Button>
        </div>
